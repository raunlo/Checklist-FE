...
export default class GameController {
  ...
  birdCol: number | undefined; 
  birdRow: number | undefined;
  bird: HTMLDivElement | undefined; 

  constructor(model: gameBrain, viewContainer: HTMLDivElement, elementOffset: number) {
    this.model = model; 
  ...
  getBoardHtml(gameBrain: gameBrain) {
    ...
    gameBrain.getGameBoard().forEach((rowData: any) => {
      ...
      rowElem.dataset.pathPosition = this.model.getDefaultPathPosition().toString();
  ...
  generateNewColumn(rowData: any, gameBrain: gameBrain) {
    ...
    rowData.forEach((colData: number) => {
  ...
  animate(content: any, gameBrain: gameBrain, isBirdAlive=true) {
    setTimeout(() => {
      ...
      if (child) {
        ...
        if (isBirdAlive) {
          ...
          this.model.submitScore(playerName ?? "anonymous");
        }
      }
  ...
  styleBirdElem(isBirdAlive: boolean) {
    if (isBirdAlive) {
